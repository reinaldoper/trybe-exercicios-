...
console.log(fullNames);
  ...
  //agora usando o map na mesma função persons
  const pessoa = [
    { firstName: 'Maria', lastName: 'Ferreira' },
    { firstName: 'João', lastName: 'Silva' },
    { firstName: 'Antonio', lastName: 'Cabral' },
  ...
  const nomeNovo = pessoa.map((person) => `${person.firstName} ${person.lastName}`);
  
console.log(nomeNovo); // [ 'Maria Ferreira', 'João Silva', 'Antonio Cabral' ]
  
  //passando para negativos
...
for (let index = 0; index < numberss.length; index += 1) {
  if (numberss[index] > 0) {
    negativeNumbers.push(numberss[index] * -1);
  } else {
    negativeNumberss.push(numbers[index]);
  }
}

console.log(negativeNumberss); // [ -1, -2, -3, -4, -5 ]
